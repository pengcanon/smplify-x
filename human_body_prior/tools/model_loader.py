...
def exprdir2model(expr_dir, model_cfg_override: dict = None):
    ...
    fname = trained_weights_fname.split('\\')
    yamldir = osp.join('\\'.join(fname[:1]), '*.yaml')
    model_cfg_fname = glob.glob(yamldir) #used to replace the above line
    model_cfg_fname = pathlib.Path(model_cfg_fname[0])
